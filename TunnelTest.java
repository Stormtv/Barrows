	...
	public void onPaint(Graphics g) {
		...
		if (doors != null) {
			for (RSObject door:doors){
				g.drawString(""+door.getModel().getPoints().length, 13, 55+20*i);
				i++;
			}
			...
			for (RSObject door : doors) {
				if (PathFinding.canReach(door.getPosition(), true)) {
					actions = door.getDefinition().getActions();
					modelIDs = door.getDefinition().getModelIDs();
					if (actions != null) {
						if (actions.length == 0) {
					...
					if (modelIDs != null) {
						if (modelIDs.length == 0) {
							g.setColor(Color.WHITE);
							g.drawString("Model Length is 0",363,55+20*i);
							Point p = Projection.tileToMinimap(door
									.getPosition());
							if (Projection.isInMinimap(p)) {
								g.setColor(Color.RED);
